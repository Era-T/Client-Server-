...
try:
   serverS.bind((serverHost,serverPort))
...
#support handling multiple clients
def client_thread(clientS,address):
    while clientS: 

        kerkesa = clientS.recv(1024)
        kerkesa=str(kerkesa.decode('utf-8'))
        kerkesa=kerkesa.split();
        print('Kerkesa nga klienti: ' +kerkesa[0])
        ...
        print('Pergjigjja nga server: ' +str(pergjigjja))
        clientS.send(str.encode(pergjigjja)) 
...
while True:
    ...
    threading.Thread(target=client_thread,args=(clientS,address)).start()
